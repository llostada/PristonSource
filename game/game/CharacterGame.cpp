...
void CharacterGame::OnCharacterUpdateData()
{
	...
	//T5
	SetFreeSkillPointForTier5( 0 ); //0 aqui lostada
	//ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: inicio");
	if 
		(TRUE)
		//( CQUESTGAME->GetQuestInstance()->IsDoneQuest( QUESTID_T5QuestArena ) )
	{
		int iPoints = 0;
		ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: dentro da quest");
		if ( pcUnitData->sCharacterData.iLevel >= 100 )
			ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: LEVEL100");
			iPoints++;
		...
		ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> buuceta FSP: %d", iPoints);
