...
BOOL SkillManager::OnBuySkill()
{
	if ( CURRENTSKILL )
	{
		...
		{
			//Locked
			ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: buy skill1");
			if ( GM_MODE == FALSE ) //LOSTADA FSP
			{
				ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: buy skill2");
			...
			if ( 
				(CHARACTERGAME->GetFreeSkillPointForTier5() > 0) 
			&& (
				(CURRENTSKILL->sSkillInfo.iSkillLevelRequired + (CURRENTSKILL->iLevel * 2)) <= PLAYERDATA->iLevel) 
				)
			{
				ChatGame::AddChatBoxTextEx(CHATCOLOR_Error, "> FSP: buy skill4");
				//Skill Price
